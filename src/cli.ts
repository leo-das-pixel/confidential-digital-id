/**
 * CLI for interacting with confidential-digital-id contract
 */
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║                   confidential-digital-id CLI                           ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    while (running) {
      console.log('─── Menu ───────────────────────────────────────────────────────');
      console.log('  1. Verify credential');
      console.log('  2. Read current state');
      ...
      switch (choice.trim()) {
        case '1': {
          const secret = await rl.question('  Enter your private credential secret: ');
          console.log('\n  Submitting transaction (this may take 30-60 seconds)...');
          try {
            const tx = await deployed.callTx.verifyCredential(secret);
            console.log(`\n  ✅ Credential verified!`);
        ...
        case '2': {
          ...
          try {
            ...
            if (contractState) {
              ...
              const name = Buffer.from(ledgerState.credentialName).toString();
              const count = ledgerState.verificationCount;
