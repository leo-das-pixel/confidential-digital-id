...
function toneFor(kind: ActivityKind): 'neutral' | 'accent' | 'ok' | 'warn' | 'danger' {
  ...
  if (kind.startsWith('verify') || kind.startsWith('deploy')) return 'accent';
  return 'neutral';
}
...
export function HistoryPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('cdi:activity', onChange);
    return () => window.removeEventListener('cdi:activity', onChange);
  }, []);
  ...
  return (
    <div>
      <PageHeader
        title="History"
        description="Local activity for wallet sessions and credential proofs."
        action={
          <Button
            variant="outline"
            size="sm"
      ...
      <Surface className="!p-0 overflow-hidden">
        {events.length === 0 ? (
          <p className="p-5 text-sm text-[var(--ink-muted)]">No activity yet.</p>
        ) : (
